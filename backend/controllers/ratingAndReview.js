...
exports.getAllRatingReview = async(req, res)=>{
    try{
        ...
        console.log('Error while fetching all ratings:', error.message);
        // Return mock reviews if DB is not available
        const mockReviews = [
            {
                rating: 5,
            ...
            {
                rating: 4,
                review: 'Great content and instructor.',
                user: { firstName: 'Jane', lastName: 'Smith', email: 'jane@example.com' },
                course: { courseName: 'Data Science Fundamentals' }
            }
        ];
        return res.status(200).json({
            success: true,
            data: mockReviews,
            message: 'Mock reviews fetched successfully (DB not connected)'
        })
    }
}
